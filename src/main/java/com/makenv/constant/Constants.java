package com.makenv.constant;

/**
 * Created by wgy on 2016/8/18.
 */
public class Constants {

    public static final String TOKEN = "token";

    public static final String REGION = "city";

    public static final String STATION = "station";

    public static final String PUBLIC_REGIONCODE = "999999";


    //<-----  redisKey ----->
    public static final String VIRTUAL_SITES= "key:virtualSite";

    public static final String RANK_KEY = "key:rank";

    public static final String QUALITY_KRY = "key:quality";

    public static final String HEV_KEY = "key:hev";

    public static final String AVG_MON_KEY="key:mon:avg";

    public static final String MIN_MAX_KEY="key:max-min:";

    public static final String RANK_ALL_KEY = "key:rankAll";

    public static final String AVG_DATE_KEY="key:date:avg&max_min";

    public static final String RANK_MONTH = "key:rank:month";

    public static final String RANK_DAY = "key:rank:day";

    public static final String RANK_HOUR = "key:rank:hour";

    public static final String RANK_YEAR = "key:rank:year";


}
